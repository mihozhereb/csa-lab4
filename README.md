...
- Поддерживается только абсолютная адресация.
- Ввод-вывод реализован через memory-mapped I/O по адресам `0x8` и `0xC`.
- Прерывания используют вектор по адресу `0x4`; `di` и `ei` запрещают и разрешают обработку прерываний. По умолчанию прерывания запрещены.
...
`0000 0000` (0x00) -- `nop` -- пустая операция
